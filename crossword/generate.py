...
class CrosswordCreator():
    ...
    def consistent(self, assignment):
        ...
        """
        for v, assign in assignment.items():
            if len(assign) != v.length:
                return False

            for i in self.crossword.neighbors(v):
                pair = self.crossword.overlaps[v, i]
                if i in assignment:
                    if assignment[i][pair[1]] != assign[pair[0]]:
                         return False
    ...
    def select_unassigned_variable(self, assignment):
        ...
        for i in self.domains.keys():
            if i not in assignment.keys():
                length = len(self.crossword.neighbors(i))
                if length > max_degree:
                    max_degree = length
                    to_return.clear()
                    to_return.append(i)
                elif length == max_degree:
                    to_return.append(i)
        return to_return[0]
